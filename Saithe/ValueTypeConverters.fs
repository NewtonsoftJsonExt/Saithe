...
type ValueTypeConverter<'T>() = 
  ...
  override this.CanConvertFrom(context, sourceType) = (sourceType = mapping.PropertyType)
...
type public ValueTypeJsonConverter<'T>() = 
  ...
  override this.ReadJson(reader, objectType, existingValue, serializer) = 
    if (objectType = t) then 
      ...
      mapping.Parse(v)
    else failwithf "Cant handle type %s, expects %s" (objectType.Name) (t.Name)
  
  override this.WriteJson(writer, value, serializer) = 
    if (value :? 'T) then writer.WriteValue(mapping.ToRaw(value :?> 'T))
    else failwithf "Cant handle type %s, expects %s" (value.GetType().Name) (t.Name)
